...
var L = function (selector) {
  ...
  // Additional check for element-like objects (for mocks in tests)
  if (
    selector &&
    typeof selector === "object" &&
    typeof selector.indexOf !== "function"
  ) {
    return selector;
  }
...
L.getPercentage = function (value, minRange, maxRange) {
  return (
    Math.round(((value - minRange) / (maxRange - minRange)) * 100 * 1000) / 1000
  );
};
...
L.createSelectItems = function (listValues, listHtml, defaultOption) {
  ...
  for (var i = 0; i < listValues.length; i++) {
    var html = hasHtmlFn
      ? listHtml(listValues[i])
      : hasHtml
      ? listHtml[i]
      : L.translate(listValues[i]);
  ...
  if (defaultOption) {
    result =
      ...
      L.translate("SELECT_SPECIAL_KEY") +
      "</option>\n" +
      result;
  }
  return result;
...
L.flattenArrayDeep = function (arr) {
  return arr.reduce(
    (acc, e) =>
      Array.isArray(e) ? acc.concat(L.flattenArrayDeep(e)) : acc.concat(e),
    [],
  );
};
