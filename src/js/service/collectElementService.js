...
collectElementService.initWithGrid = function (gridData, dontAutoPredict) {
    ...
    if (element && element.type === GridElement.ELEMENT_TYPE_COLLECT) {
      ...
      dictionaryKey =
        ...
        copy.actions.reduce((total, action) => {
          let dictKey = GridActionPredict.getModelName()
  ...
  if (gridData.keyboardMode) {
    keyboardLikeFactor =
      gridData.keyboardMode === GridData.KEYBOARD_DISABLED ? 0 : 1;
  } else {
...
collectElementService.doCollectElementActions = async function (
  action,
  gridElement,
) {
  if (!action) {
...
collectElementService.addWordFormTagsToLast = function (
  tags,
  toggle,
  skipElementId,
) {
  ...
  if (targetElement && !targetElement.wordFormFixated) {
    ...
    elementCopy.wordFormTags = stateService.mergeTags(
      elementCopy.wordFormTags,
      tags,
      toggle,
    );
    let newLabel = stateService.getWordForm(elementCopy, {
      searchTags: elementCopy.wordFormTags,
      searchSubTags: true,
    });
...
async function updateCollectElements(isSecondTry) {
  ...
  for (let collectElement of registeredCollectElements) {
    let txtBackgroundColor =
      metadata.colorConfig.gridBackgroundColor || "#ffffff";
    let imageMode = isSeparateMode(collectElement.mode);
    let outerContainerJqueryElem = $(
      `#${collectElement.id} .collect-outer-container`,
    );
    let darkMode =
    ...
    let textColor = darkMode
      ...
      : constants.DEFAULT_ELEMENT_FONT_COLOR;
    ...
    if (!imageMode) {
      ...
      $(`#${collectElement.id}`).attr(
        "aria-label",
        `${text}, ${i18nService.t("ELEMENT_TYPE_COLLECT")}`,
      );
      ...
      while (maxImages < imageCount && !useSingleLine) {
        numLines++;
        maxImages =
          Math.floor(width / ((imgContainerHeight * maxImgRatio) / numLines)) *
          numLines;
      }
      ...
      for (const [index, collectedElement] of collectedElements.entries()) {
        ...
        if (collectElement.showFullLabels) {
          let textWidth = fontUtil.getTextWidth(
            label,
            outerContainerJqueryElem[0],
            `${textHeight}px`,
          );
        ...
        html += `<div id="collect${index}" style="display: flex; flex:0; justify-content: center; flex-direction: column; padding: ${imgMargin}px; color: ${textColor}; ${
          marked
            ? `background-color: ${darkMode ? "darkgreen" : "lightgreen"};`
            : ""
        }" title="${label}">
      ...
      let additionalCSS = useSingleLine
  ...
  $(document).trigger(constants.EVENT_COLLECT_TEXT_CHANGED, [getPrintText()]);
}
...
function getOutputObject(element, options) {
  options = options || {};
  let audioAction = element.actions.filter(
    (a) => a.modelName === GridActionAudio.getModelName(),
  )[0];
  ...
  let customSpeakAction = element.actions.filter(
  ...
  if (!text && customSpeakAction) {
    ...
    text = i18nService.getTranslation(customSpeakAction.speakText, {
      lang: lang,
    });
  }
  if (!text) {
...
function getSpeakArray(options) {
  ...
  options.inlcudeCorrectedGrammar =
    options.inlcudeCorrectedGrammar !== undefined
      ? options.inlcudeCorrectedGrammar
      : true;
...
function getPrintText(options) {
  ...
  options.dontIncludePronunciation =
    options.dontIncludePronunciation !== undefined
      ? options.dontIncludePronunciation
      : true;
  options.inlcudeCorrectedGrammar =
    options.inlcudeCorrectedGrammar !== undefined
      ? options.inlcudeCorrectedGrammar
      : true;
  let textArray = collectedElements.map(
    (e) => getOutputObject(e, options).text,
  );
  let returnValue = options.trim
    ? textArray.join(" ").trim()
    : textArray.join(" ");
...
$(window).on(constants.ELEMENT_EVENT_ID, function (event, element) {
  if (
    ...
    new Date().getTime() - lastCollectTime < duplicatedCollectPause
  ) {
    return;
  }
  lastCollectId = element.id;
  ...
  let navigateAction = getActionOfType(
    element,
    GridActionNavigate.getModelName(),
  );
  if (
    navigateAction &&
    getLabel(element).length !== 1 &&
    !navigateAction.addToCollectElem
  ) {
    return; // no adding of text if the element contains an navigate action and it's no single keyboard character
  }
  let wordFormActions = getActionsOfType(
    element,
    GridActionWordForm.getModelName(),
  );
  if (
    wordFormActions.length > 0 &&
    wordFormActions.some(
...
async function getMetadataConfig() {
  ...
  duplicatedCollectPause =
    _localMetadata.inputConfig.globalMinPauseCollectSpeak || 0;
