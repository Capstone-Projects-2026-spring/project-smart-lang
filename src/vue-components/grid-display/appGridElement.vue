<template>
  <div
    v-if="element.type !== GridElement.ELEMENT_TYPE_UI_FILLER"
    role="button"
    class="element-container"
    ref="container"
    tabindex="40"
    :aria-label="getAriaLabel(element)"
    :data-empty="isEmpty(element)"
    :style="`margin: ${elementMarginPx}px; border-radius: ${borderRadiusPx}px; cursor: ${cursorType};
         border: ${borderWidthPx}px solid ${getBorderColor(
      element,
    )}; background-color: ${backgroundColor}; font-family: ${
      metadata.textConfig.fontFamily
    }; color: ${fontColor}`"
  >
    <grid-element-normal
      v-if="element.type === GridElement.ELEMENT_TYPE_NORMAL"
      :grid-element="element"
      :metadata="metadata"
      :container-size="calculatedSize"
      v-bind="$props"
      aria-hidden="true"
    />
    <grid-element-collect
      v-if="element.type === GridElement.ELEMENT_TYPE_COLLECT"
      :metadata="metadata"
      aria-hidden="true"
    />
    <grid-element-predict
      v-if="element.type === GridElement.ELEMENT_TYPE_PREDICTION"
      :grid-element="element"
      :metadata="metadata"
      :container-size="calculatedSize"
      v-bind="$props"
    ...
    />
    <grid-element-live
      v-if="element.type === GridElement.ELEMENT_TYPE_LIVE"
      :grid-element="element"
      :metadata="metadata"
      :container-size="calculatedSize"
      v-bind="$props"
      aria-hidden="true"
    />
    <grid-element-child-placeholder
      v-if="element.type === GridElement.ELEMENT_TYPE_DYNAMIC_GRID_PLACEHOLDER"
    />
    <grid-element-hints
      :grid-element="element"
      :metadata="metadata"
    ...
    <div
      ...
      class="ui-resizable-handle ui-icon ui-icon-grip-diagonal-se"
      style="
        position: absolute;
        z-index: 2;
        bottom: 0;
...
export default {
  ...
  data() {
    return {
      ...
      resizeObserver: null,
      elementMarginPx: fontUtil.pctToPx(
        this.metadata.colorConfig.elementMargin,
      ),
  ...
  computed: {
    calculatedSize() {
      return {
        width:
          this.oneElementSize.width * this.element.width -
          2 * this.getElementMargin() -
          2 * this.getBorderWidth(),
        height:
          this.oneElementSize.height * this.element.height -
          2 * this.getElementMargin() -
          2 * this.getBorderWidth(),
      };
    },
    ...
    fontColor() {
      ...
      if (
        ...
        [constants.COLORS.BLACK, constants.COLORS.WHITE].includes(
          this.metadata.textConfig.fontColor,
        )
      ) {
  ...
  methods: {
    getBorderColor(element) {
      ...
      if (
        ...
        // element.borderColor only used for color mode "both"
        return element.borderColor;
      }

      let color = this.metadata.colorConfig.elementBorderColor;
      if (
        this.metadata.colorConfig.elementBorderColor ===
        constants.DEFAULT_ELEMENT_BORDER_COLOR
      ) {
        let backgroundColor =
          this.metadata.colorConfig.gridBackgroundColor ||
          constants.COLORS.WHITE;
        color = fontUtil.getHighContrastColor(
          backgroundColor,
          constants.COLORS.WHITESMOKE,
      ...
      if (this.metadata.colorConfig.colorMode === ColorConfig.COLOR_MODE_BOTH) {
        ...
        if (
          colorScheme &&
          colorScheme.customBorders &&
          colorScheme.customBorders[element.colorCategory]
        ) {
        ...
        let adjustment = fontUtil.isHexDark(bgColor)
          ? absAdjustment * 1.5
          : absAdjustment * -1;
    ...
    isEmpty(element) {
      if (element.type === GridElementModel.ELEMENT_TYPE_NORMAL) {
        return (
          !stateService.getDisplayText(element.id) &&
          (!element.image || (!element.image.url && !element.image.data))
        );
      }
      return false;
    },
    getAriaLabel(gridElem) {
      ...
      let speakCustomAction = gridElem.actions.filter(
        (a) => a.modelName === GridActionSpeakCustom.getModelName(),
      )[0];
      if (!ariaLabel && speakCustomAction) {
        ariaLabel =
          i18nService.getTranslation(speakCustomAction.speakText) + ", ";
      }
    ...
    async recalculate() {
      ...
      this.borderRadiusPx = fontUtil.pctToPx(
        this.metadata.colorConfig.borderRadius,
      );
    },
    getElementMargin() {
